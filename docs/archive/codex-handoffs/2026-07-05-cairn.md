# Codex Session Handoff — Cairn

> Archived 2026-07-31 because it contains durable historical context, then moved into the archived Codex handoff namespace during the assistant-workflow conversion. Work items were migrated to the [Cairn Roadmap](https://github.com/users/ehussong/projects/6); this file is no longer a status source.
...
The design deliverable is archived as **`docs/archive/2026-07-01-cairn-store-design.md`** (Draft v2). It contains all seven required artifacts: schema with full DDL, ingestion pipeline + connector contract with change detection and delete propagation per source, chunking strategy, MCP tool contracts with the hybrid RRF SQL, backup design for both tiers, operational plan, and a 6-phase build sequence. §10 answers the brief's four open questions with recommendations.
...
- At the time of this handoff, the Store design, this handoff, `.serena/`, and the migrated Router files were untracked. Their current archived locations supersede the original paths.
